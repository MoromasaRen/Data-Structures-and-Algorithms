...
/* Priority Queue using a Partially Ordered Tree (Heap)
   This version is a MIN-Priority Queue:
...
int main() {
    ...
    printf("\nPeek (highest priority / min) = %d\n", peek(PQ));

    printf("\n=== DEQUEUE (remove highest priority / min) ===\n");
    while (!isEmptyPQ(PQ)) {
        int x = dequeue(&PQ);
        printf("Dequeued: %d | ", x);
        displayPQ(PQ);
...
/* Remove the root (min) (bubble down) */
int dequeue(PriorityQueue *PQ) {
    if (isEmptyPQ(*PQ)) return -1;
...
void heapifyDown(PriorityQueue *PQ, int index) {
    while (1) {
        ...
        if (left < PQ->size && PQ->elem[left] < PQ->elem[smallest]) smallest = left;
        if (right < PQ->size && PQ->elem[right] < PQ->elem[smallest]) smallest = right;

        if (smallest == index) break;

        int temp = PQ->elem[index];
        PQ->elem[index] = PQ->elem[smallest];
        PQ->elem[smallest] = temp;
